class SwipeJob < ApplicationRecord
  ...
  startDate = currentDate.change(hour: 0, minute: 0, second:0)

  scope :all_jobs, -> { where.not(job_type: 'status_check') }
  ...
  scope :completed, -> { where(status: 'completed').where(failed_reason: 'finished running job') }
  ...
  scope :proxy_error, -> { where(status: 'completed').where(failed_reason: 'proxy_error') }
  scope :socket_connection_error, -> { where(status: 'completed').where(failed_reason: 'socket_connection_error') }
  scope :my_jobs, -> { where('schedule_id IS NULL').where('scheduled_at IS NULL').where.not(job_type: 'status_check')}
  scope :cancelled, -> { where(status: 'cancelled') }
  scope :today, -> { where('completed_at > ? or scheduled_at > ? or created_at > ?', startDate, startDate, startDate) }
